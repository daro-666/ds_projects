...
def nmf_predict(user:dict, n_recs=10, fillNaValue=0.0) -> pd.DataFrame:
    '''successive use of functions to make recommendation. see used functions descriptions for details
    '''
    user_df = user_dicttoDF(user)
    user_df_filled = fill_user_df(user_df, fillNaValue)
...
def nmf_preds_to_list(output:pd.DataFrame) -> list:
    '''
    '''
    output_list = []

    for i in range(output.shape[0]):
        output_list.append(f"{output['title'][i+1]}, {output['predicted_rating'][i+1]}")

    return output_list


def user_val_str_to_flt(user:dict) -> dict:
    '''
    '''
